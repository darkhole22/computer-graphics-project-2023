...
GameManager::GameManager(const TerrainGenerationConfig &terrainConfig)
    : m_Scene(Application::getScene()),
      ...
      m_EnemyFactory(
          m_EnemyWaveSize,
          glm::rotate(glm::mat4(1), glm::half_pi<f32>(), glm::vec3(0, 1, 0))),
  ...
  EventBus::addCallback(
      [this](DoubleScoreStarted e) { onDoubleScoreStarted(e); });
...
void GameManager::onDoubleScoreStarted(DoubleScoreStarted e) {
  m_Scene->makeTimer(e.duration)
      ->addCallback([this](const TimerTimeoutEvent &) {
