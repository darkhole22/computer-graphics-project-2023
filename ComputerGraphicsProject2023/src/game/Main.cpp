...
private:
  ...
  void setupLight() {
    ...
    m_Scene->getWorld()->directLight.direction =
    ...
    auto lightFun = [this](f32 angle) {
      ...
      m_Scene->getWorld()->directLight.color =
          glm::vec4(1.0f, 0.2f + 0.7f * std::pow(sin(angle), 3.0f),
                    std::pow(sin(angle), 5.0f) * 0.9f, 1.0f);
      m_Scene->getWorld()->directLight.direction = glm::vec3(
          cos(angle) * cos(hAngle), sin(angle) * cos(hAngle), sin(hAngle));
    };
    ...
    auto lightInvFun = [this](f32 angle) {
      ...
      m_Scene->getWorld()->directLight.color =
          static_cast<f32>(1.0f - 0.5f * sin(angle)) *
    ...
    directLightTween->addCallbackTweener(
        [this]() { m_Scene->setSkybox(SKYBOX_MORNING); });
    directLightTween->addMethodTweener<f32>(lightFun, 0.0f, glm::radians(30.0f),
                                            15.0f);
    directLightTween->addCallbackTweener(
        [this]() { m_Scene->setSkybox(SKYBOX_DAY); });
    directLightTween->addMethodTweener<f32>(lightFun, glm::radians(30.0f),
                                            glm::radians(150.0f), 60.0f);
    directLightTween->addCallbackTweener(
        [this]() { m_Scene->setSkybox(SKYBOX_SUNSET); });
    directLightTween->addMethodTweener<f32>(lightFun, glm::radians(150.0f),
                                            glm::radians(180.0f), 10.0f);
    directLightTween->addCallbackTweener(
        [this]() { m_Scene->setSkybox(SKYBOX_MIDNIGHT); });
    directLightTween->addMethodTweener<f32>(lightInvFun, glm::radians(180.0f),
                                            glm::radians(30.0f), 10.0f);
    directLightTween->addCallbackTweener(
        [this]() { m_Scene->setSkybox(SKYBOX_NIGHT); });
    directLightTween->addMethodTweener<f32>(lightInvFun, glm::radians(150.0f),
                                            0.0f, 50.0f);
  }
...
int main() {
  ...
  try {
    Ref<Application> app; // The game must be destroyed before the application
    {
      ...
      app = Application::launch(game,
                                vulture::AppConfig{"Vulture demo", 800, 600});
    }
