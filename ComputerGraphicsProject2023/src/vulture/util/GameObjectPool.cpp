...
GameObjectPool::GameObjectPool(u32 initialSize, const String& modelName, const String& textureName, const glm::mat4& loadTransform) :
	m_ModelName(modelName), m_TextureName(textureName)
{
	for (u32 i = 0; i < initialSize; i++)
	{
		m_ObjectQueue.push(makeRef<GameObject>(m_ModelName, m_TextureName, loadTransform));
	}
}
