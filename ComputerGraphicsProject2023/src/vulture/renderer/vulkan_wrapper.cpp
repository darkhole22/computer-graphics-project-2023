...
namespace vulture
{
	...
	Device::Device(const Instance &instance, const Surface &surface) : m_PhysicalDevice(PhysicalDevice::pickDevice(instance, surface))
	{
	...
	CommandBuffer::CommandBuffer() : m_Handle(VK_NULL_HANDLE), m_Device(nullptr)
	{
	}
	...
	Image::Image() : m_Handle(VK_NULL_HANDLE), m_Memory(VK_NULL_HANDLE), m_View(VK_NULL_HANDLE), m_Device(nullptr), m_Format(VK_FORMAT_UNDEFINED)
	{
	}
	...
	Image::Image(VkImage image, const Device &device, VkFormat format) : m_Memory(VK_NULL_HANDLE), m_View(VK_NULL_HANDLE), m_Format(format)
	{
		m_Handle = image;
		m_Device = &device;
	...
	Image::Image(const Device &device, uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImageAspectFlags aspectFlags) : m_Format(format), m_Width(width), m_Height(height)
	{
		m_Device = &device;
	...
	RenderPass::RenderPass(const Device &device, const Surface &surface) : m_Device(&device)
	{
	...
	SwapChain::SwapChain(const Device &device, const Surface &surface, const RenderPass &renderPass) : m_Device(&device), m_Surface(&surface), m_RenderPass(&renderPass)
	{
		create();
	...
	DescriptorSetLayout::DescriptorSetLayout(const Device &device) : m_Handle(VK_NULL_HANDLE), m_Device(&device)
	{
	}
	...
	Shader::Shader(const Device &device, const String &name) : m_Device(&device)
	{
	...
	Buffer::Buffer() : m_Handle(VK_NULL_HANDLE), m_Memory(VK_NULL_HANDLE), m_Device(nullptr), m_Size(0), m_Data(nullptr)
	{
	}
	...
	Buffer::Buffer(const Device &device, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, void *data) : m_Device(&device), m_Size(size), m_Data(data)
	{
	...
	Buffer::Buffer(const Device &device, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties) : Buffer(device, size, usage, properties, nullptr)
	{
	}
	...
	Texture::Texture(const Device &device, const String &path) : m_Device(&device)
	{
	...
	DescriptorSet::DescriptorSet(DescriptorPool &pool, const DescriptorSetLayout &layout, const std::vector<DescriptorWrite> &descriptorWrites) : m_Pool(&pool), m_Layout(&layout), m_DescriptorWrites(descriptorWrites)
	{
		create();
	}
	...
	DescriptorPool::DescriptorPool(const Device &device, uint32_t frameCount) : m_Device(&device), m_FrameCount(frameCount)
	{
	}
